...
class AddAppointment extends React.Component {
  ...
  onSubmit = (e)=>
  {
      ...
      this.props.showError({errorMsg:"Fill All Details", errorLogo:"❌"});
        ...
        axios.post("/addAppointment", this.state).then((response)=>this.props.showError({errorMsg:"Appointment Added Successfully", errorLogo:"✅"})).catch((err)=>this.props.showError({errorMsg:"Appointment cannot be added", errorLogo:"❌"}));
  ...
  render() {
    return (
      <div className={classes.AddAppointment_root}>
        <h2 style={{ textDecoration: "underline" }}>Add Appointment</h2>
        <br />
        <input
        ...
        <br/>
        <br/>
        <Button text="Fix Appointment" onClick = {this.onSubmit}/>
      </div>
